...
-(id)objFromString:(NSString*) jsonString withClass:(id) myClass{
    ...
    while (class_getSuperclass(myClass) != [QUEntity class]) {
...
//        NSString *propertyAttrString = [[NSString alloc] initWithCString:getPropertyType(property) encoding:NSUTF8StringEncoding];;
...
//        NSString *newPropertyNameString = pMethod ? [NSString stringWithFormat:@"%@%@%@Entity",PROJECTNAME,pMethod,upPropertyNameString]:[NSString stringWithFormat:@"%@%@Entity",PROJECTNAME,upPropertyNameString];
        NSString *newPropertyNameString = pMethod ? [NSString stringWithFormat:@"%@%@%@Entity",PROJECTNAME,pMethod,upPropertyNameString]:nil;
        //判断名为newPropertyNameString的类是否存在
        Class pkClass=NSClassFromString(newPropertyNameString);
        if(!pkClass){
            pkClass=NSClassFromString([NSString stringWithFormat:@"%@.%@",kAppName,newPropertyNameString]);//兼容swift
        }
        //set方法
...
-(NSString *)stringFromObjc:(id )obj{
    ...
    while (class_getSuperclass(cls) != [QUEntity class] && class_getSuperclass(cls) != [QUMockParam class] && cls != [QUMockParam class]  ) {
...
-(NSString *)stringFromObjcs:(id )obj withSuperObjc:(id) superClass{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        //以类名方式拼接大写后的字符串
        //NSString *newPropertyNameString = pMethod ? [NSString stringWithFormat:@"%@%@%@Entity",PROJECTNAME,pMethod,upPropertyNameString]:[NSString stringWithFormat:@"%@%@Entity",PROJECTNAME,upPropertyNameString];
        NSString *newPropertyNameString = pMethod ? [NSString stringWithFormat:@"%@%@%@Entity",PROJECTNAME,pMethod,upPropertyNameString]:@"";
        //get方法
