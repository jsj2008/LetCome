...
/**
 @param entity 带插入的实体
 @param sectionClass 映射的sectionClass
 @param beforeEntity 插入在某个entity之前
 */
-(void)addEntity:(QUEntity*)entity withSection:(Class)sectionClass beforeEntity:(QUEntity*)beforeEntity{
    ...
    NSInteger index=[self.pEntityList indexOfObject:beforeEntity.group];
    
    if(index>=0)
        index++;
    
    [self.pEntityList insertObject:list atIndex:index-1];
