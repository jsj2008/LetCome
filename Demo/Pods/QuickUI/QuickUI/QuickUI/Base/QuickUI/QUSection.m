...
-(void)fixLabelWithSmallScreen{
    [self fixLabelWithView:self];
}

-(void)fixLabelWithView:(UIView*)view{
    if (iPhone4 || iPhone5) {
...
-(void)sectionWillLoad{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(QUTableViewSelectedCellInsertNotify:) name:QU_TABLE_CELL_SELECTED_INSERT_NOTIFY object:nil];
}
...
-(void)QUTableViewSelectedCellInsertNotify:(NSNotification*)notification
{
    dispatch_async(dispatch_get_main_queue(), ^{
        Class sectionClass=[notification.userInfo objectForKey:@"sectionClass"];
        ...
        if([self isMemberOfClass:sectionClass])
        {
            if(!tag)  // 没有设置标记，则发送给匹配的Section
            {
                [self.pAdaptor.pTableView beginUpdates];
                NSIndexPath *index = [NSIndexPath indexPathForRow:self.rows+1 inSection:self.section];
                [self.pAdaptor.pTableView insertRowsAtIndexPaths:@[index] withRowAnimation:[animation integerValue]];
                [self.pAdaptor.pTableView endUpdates];
            }
            else      // 如果设置了标记，直接发送调用
            {
                if(self.entity.tag==[tag integerValue])
                {
                    ...
                    NSIndexPath *index = [NSIndexPath indexPathForRow:self.rows+1 inSection:self.section];
                    [self.pAdaptor.pTableView insertRowsAtIndexPaths:@[index] withRowAnimation:[animation integerValue]];
//                    [self.pAdaptor.pTableView insertRowsAtIndexPaths:@[self.pIndexPath] withRowAnimation:[animation integerValue]];
                    [self.pAdaptor.pTableView endUpdates];
                }
            }
        }
