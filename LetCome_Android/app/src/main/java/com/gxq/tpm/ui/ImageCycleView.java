...
public class ImageCycleView extends LinearLayout {
	...
	public ImageCycleView(Context context, AttributeSet attrs) {
		...
		LayoutInflater.from(context).inflate(R.layout.cycle_view_content, this);
	 ...
	 */
	public void setImageResources(List<AdInfo.Record> infoList, ImageCycleViewListener imageCycleViewListener) {
		// 清除所有子视图
...
//			int imagePadding = (int) (mScale * 5 + 0.5f);
			...
			if (i == 0) {
				mImageViews[i].setBackgroundResource(R.drawable.item_sliderw_s);
			} else {
				mImageViews[i].setBackgroundResource(R.drawable.item_sliderw);
			}
	...
	private Runnable mImageTimerTask = new Runnable() {
		...
		public void run() {
			if (mImageViews != null) {
				...
				mBannerPager.setCurrentItem(mImageIndex,false);
			}
		}
	};
...
//			}
			if(index == mImageViews.length + 1){
				...
				index = mImageViews.length;
			}
			// 设置图片滚动指示器背景
			mImageIndex = index;
			index -= 1;
			mImageViews[index].setBackgroundResource(R.drawable.item_sliderw_s);
	...
	private class ImageCycleAdapter extends PagerAdapter {
		 ...
		 * 图片资源列表
		 */
		private List<AdInfo.Record> mAdList = new ArrayList<AdInfo.Record>();
		...
		public ImageCycleAdapter(Context context, List<AdInfo.Record> adList, ImageCycleViewListener imageCycleViewListener) {
			mContext = context;
			mAdList = adList;
	...
	public static interface ImageCycleViewListener {
		 ...
		 * @param imageView
		 */
		public void onImageClick(AdInfo.Record record, int postion, View imageView);
	}
