...
public class UserPrefs extends BasePrefs {
	...
	public UserInfo getUserInfo() {
		return readBaseRes(USER_INFO, UserInfo.class);
	}
	...
	public void setIniPatch(IniPatch patch) {
		writeBaseRes(INI_PATCH, patch);
		save();
	}
	
	public String getStockListVersion() {
		return getString(STOCK_LIST_VERSION, "0");
	}
	
	public void setStockListVersion(String version) {
		putString(STOCK_LIST_VERSION, version);
		save();
	}
	
	public IniPatch getIniPatch() {
		return readBaseRes(INI_PATCH, IniPatch.class);
	}
	
	public LockPrefs getLockPrefs() {
		if (mLockPrefs == null || mLockPrefs.getUid() != getUid()) {
			mLockPrefs = LockPrefs.get(mContext, getUid());
		}
		return mLockPrefs;
	}
	...
	public SearchStock getSearchStock() {
		return readBaseRes(SEARCH_STOCK, SearchStock.class);
	}
	
	public void setSearchStock(SearchStock searchStock) {
		writeBaseRes(SEARCH_STOCK, searchStock);
		save();
	}
